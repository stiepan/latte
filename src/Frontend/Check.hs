...
data SemanticError =
  ...
  ReturnOutsideProcedure Position |
...
pIdent2Ident :: PIdent -> (Ident, Position)
pIdent2Ident (PIdent ((line, column), name)) = (Ident name, Position line column)
...
check :: Program -> Maybe SemanticError
check program =
  case result of
    Left e -> Just e
    Right _ -> Nothing
  where
    result = evalState (runExceptT (checkP program)) emptyContext
